/area/ship/kasf_corvette
	name = "KASF Corvette"
	requires_power = TRUE
	dynamic_lighting = TRUE
	no_light_control = FALSE
	base_turf = /turf/space
	flags = RAD_SHIELDED

/area/ship/kasf_corvette/portthrust
	name = "KASF Corvette - Port Thruster Pod"
	ambience = AMBIENCE_ATMOS

/area/ship/kasf_corvette/starboardthrust
	name = "KASF Corvette - Starboard Thruster Pod"
	ambience = AMBIENCE_ATMOS

/area/ship/kasf_corvette/porthangarfoyer
	name = "KASF Corvette - Port Hangar Foyer"
	ambience = AMBIENCE_HANGAR

/area/ship/kasf_corvette/starboardhangarfoyer
	name = "KASF Corvette - Starboard Hangar Foyer"
	ambience = AMBIENCE_HANGAR

/area/ship/kasf_corvette/hangar
	name = "KASF Corvette - Hangar"
	ambience = AMBIENCE_HANGAR
	sound_env = HANGAR

/area/ship/kasf_corvette/armory
	name = "KASF Corvette - Armory"
	ambience = AMBIENCE_HIGHSEC

/area/ship/kasf_corvette/cic
	name = "KASF Corvette - Command Center"
	ambience = AMBIENCE_HIGHSEC

/area/ship/kasf_corvette/mainhall
	name = "KASF Corvette - Main Hallway"
	ambience = AMBIENCE_GENERIC

/area/ship/kasf_corvette/engie
	name = "KASF Corvette - Engineering"
	ambience = AMBIENCE_ENGINEERING

/area/ship/kasf_corvette/atmos
	name = "KASF Corvette - Atmospherics"
	ambience = AMBIENCE_ATMOS

/area/ship/kasf_corvette/portwep
	name = "KASF Corvette - Secondary Armament"
	ambience = AMBIENCE_HIGHSEC

/area/ship/kasf_corvette/starboardwep
	name = "KASF Corvette - Primary Armament"
	ambience = AMBIENCE_HIGHSEC

/area/ship/kasf_corvette/dorm
	name = "KASF Corvette - Crew Quarters"

/area/ship/kasf_corvette/medbay
	name = "KASF Corvette - Medical Bay"

/area/ship/kasf_corvette/mess
	name = "KASF Corvette - Recreation Area"

/area/ship/kasf_corvette/forehall
	name = "KASF Corvette - Fore Hallway"
	ambience = AMBIENCE_GENERIC

/area/ship/kasf_corvette/cryo
	name = "KASF Corvette - Cryogenic Storage"

/area/ship/kasf_corvette/captain
	name = "KASF Corvette - Captain's Quarters"

/area/ship/kasf_corvette/washroom
	name = "KASF Corvette - Washroom"
	sound_env = SMALL_ENCLOSED

/area/ship/kasf_corvette/brig
	name = "KASF Corvette - Brig"
	ambience = AMBIENCE_GENERIC

// Shuttle
/area/shuttle/kasf_shuttle
	requires_power = TRUE
	name = "KASF Shuttle"
	icon_state = "shuttle2"
