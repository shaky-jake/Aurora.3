...
/obj/structure/survey_probe/proc/get_report(var/turf/T, var/is_exoplanet, var/is_asteroid)
	...
	if(istype(T, /turf/simulated/floor/exoplanet/desert))
		. += "<br>Signs of water depletion detected in local soil samples. May cause long-term decline in local grasses and herbivore populations."
		. += "<br>Radioactive contamination levels in local soil samples are at low, yet statistically significant levels."
		. += "<br>The largest danger to local flora is insufficient soil nutrition. Intervention may be required in the long term."
	if(istype(T, /turf/simulated/floor/exoplanet/grass/moghes/dirt))
		. += "<br>Rich subsurface mineral deposits detected."
		. += "<br>Levels of radioactive contamination in soil minimal."
		. += "<br>Acceptable nutrient depletio detected in soil samples."
	. += "<br>Atmospheric fallout levels low - intervention likely required. Rate of dispersal uncertain."
